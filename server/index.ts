    ...
    try {
      ...
      if (false) { // Temporarily disabled fallback check
        ...
        if (messageText.includes('مذاكرة') || messageText.includes('دراسة') || messageText.includes('تعلم')) {
          ...
          contextualResponse = 'العلوم مجال واسع ومثير! أي فرع من العلوم تريد أن نتناول��؟ وما المفهوم المحدد الذي تحتاج مساعدة فيه؟';
...
## منهجيتك التعليمية السعو��ية الخليجية:
...
5. حول الموضوع بطريقة طبيعية وودودة ��ون إحراج الطالب
...
أنت ترد: "نايس! تخيل معي لو عندك 8 تفاحات وتريد توزعها على 8 أشخاص بالت��اوي، كم تفاحة ل��ل شخص؟"
الطالب يجيب: "١"
...
تذكر: أنت تبني محادثة تعليمية مترابطة، ليس مجرد إجابات منفصلة.`;
      ...
      try {
        ...
        res.write(JSON.stringify({
          content: fullResponse || 'عذراً، لم أتمكن ��ن فهم سؤالك. يمكنك إعادة صياغته؟',
          isComplete: true,
          messageId: Date.now().toString(),
          sessionId: sessionId || 'session-' + Date.now(),
          userId: userId || 'user-' + Date.now(),
        ...
        if (image) {
          ...
          fallbackResponse = "أشوف إنك أرسلت صورة! للأسف ما أقدر أحللها حالياً، بس يمكنك تساعدني تساعدك أكثر:\n\n• لو هي مسألة رياضية، اكتب المسألة بالكلمات\n• لو هي رسمة علمية، وصف لي وش تشوف فيها\n• لو هي نص، انقل لي النص اللي تبي مساعدة فيه\n\nوأنا أساعدك خطوة بخطوة! 😊";
        } else {
      ...
      res.status(500).json({
        ...
        message: "��ذراً، حدث خطأ في النظام. يرجى المحاولة لاحقاً.",
      });
    }
  });
