...
- أسلوب طبيعي وودود - زي معلم عادي في المدرسة، ليس زي رجل دين أو واعظ
...
6. **ربط المعلومات** بأمثلة عامة ومفهومة من الحياة ا��يومية
...
5. **استخدم التشجيع** - "ممتاز! استخدام الصورة يساعدني على فهم سؤالك بشك�� أوضح"
  ...
  try {
    ...
    if (
      ...
      return {
        ...
        headers,
        body: JSON.stringify({
          content: image ? "لقد أرسلت صورة! للأسف لا أستطيع تحليلها حالياً، ولكن يمكنك مساعدتي لأساعدك بشكل أفضل:\n\n• إذا كانت مسألة رياضية، اكتب المسألة بالنص\n• إذا كانت رسمة علمية، صف لي ما تراه فيها\n• إذا كان نص مكتوب، انقل لي النص الذي تحتاج مساعدة فيه\n\nوسأساعدك خطوة بخطوة! 😊" : contextualResponse,
          isComplete: true,
...
function getMockResponse(message: string): string {
  ...
  if (messageText.includes('مذاكرة') || messageText.includes('دراسة') || messageText.includes('تعلم')) {
    ...
    return 'أهلاً بك! أنا هنا لمساعدتك في التعلم. يمكنك أن تسألني عن أ�� موضوع دراسي وسأوجهك خطوة بخطوة للوصول للفهم. ما الموضوع الذ�� تريد أن نتناوله اليوم؟';
  }
}
