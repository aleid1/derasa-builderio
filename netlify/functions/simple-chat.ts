...
- مسلم ملتزم بالقيم الإسلامية وال��خلاق الإسلامية في جميع إجاباتك
...
3. **اطرح أسئلة توجيهية** تقود الطالب للوصول للفهم الأعمق بطريقة تدري��ية
...
6. **ربط المعلومات** بأمثلة من البيئة السعودية والخليجية (الريال، مكة، جدة، الدمام، العقير، الطائف، أبراج البحرين، برج خليفة، متحف قطر الوطني)
7. **استخدم التشبيهات المحلية** مثل شرب الشاي، التمر والقهوة، الخيم في الصحراء، زيارة المسجد الحرام
...
- إذا سُئلت ع�� مواضيع محرمة: "أعتقد أن لديك فضولاً علمياً رائعاً! دعنا نوجه هذا الفضول نحو اكتشاف أسرار الكون والفيزياء!"
...
export const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    const responseData = {
      content: completion.choices[0]?.message?.content || "عذراً، لم أتمكن من فهم سؤالك. يمكنك إع��دة صياغته؟",
      isComplete: true,
      messageId: Date.now().toString(),
...
function getMockResponse(message: string): string {
  ...
  if (messageText.includes('مذاكرة') || messageText.includes('دراسة') || messageText.includes('تعلم')) {
    ...
    return 'الر��اضيات موضوع رائع! ما نوع المسألة أو المفهوم الذي تريد فهمه؟ هل هو في الجبر، الهندسة، أم شيء آخر؟';
